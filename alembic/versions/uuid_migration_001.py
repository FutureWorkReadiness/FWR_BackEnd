...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_specializations_specialization_id",
        "specializations",
        ["specialization_id"],
        unique=False,
    )
    op.create_index(
        "ix_specializations_name", "specializations", ["name"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "preferred_specialization_id", postgresql.UUID(as_uuid=True), nullable=True
        ),
    ...
    op.create_index(
        "ix_questions_question_id", "questions", ["question_id"], unique=False
    )

    # Create question_options table with UUID
    # New format: key (A-E), text, is_correct, rationale
    op.create_table(
        "question_options",
        sa.Column("option_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("question_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("key", sa.String(length=1), nullable=False),  # A, B, C, D, or E
        sa.Column("text", sa.Text(), nullable=False),
        sa.Column("is_correct", sa.Boolean(), nullable=True, default=False),
        sa.Column("rationale", sa.Text(), nullable=True),  # Why correct/incorrect
        sa.Column(
            "created_at",
            sa.DateTime(timezone=True),
            server_default=sa.text("now()"),
    ...
    op.create_index(
        "ix_question_options_option_id", "question_options", ["option_id"], unique=False
    )
    ...
    op.create_index(
        "ix_quiz_attempts_attempt_id", "quiz_attempts", ["attempt_id"], unique=False
    )
    ...
    op.create_index(
        "ix_journal_entries_entry_id", "journal_entries", ["entry_id"], unique=False
    )
    ...
    op.create_index(
        "ix_user_badges_user_badge_id", "user_badges", ["user_badge_id"], unique=False
    )
    ...
    op.create_index(
        "ix_peer_benchmarks_benchmark_id",
        "peer_benchmarks",
        ["benchmark_id"],
        unique=False,
    )

    # Create generation_jobs table for quiz generator job tracking
    op.create_table(
        "generation_jobs",
    ...
    op.create_index(
        "ix_generation_jobs_job_id", "generation_jobs", ["job_id"], unique=False
    )
